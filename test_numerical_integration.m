function phiphidphi = test_numerical_integration()
...
for i = 1:4
  for j = 1:4
    for k = 1:2
      for l = 1:4
	printf('%i %i %i %i\n', i, j, k, l);
	phiphidphi(i, j, k, l) = quadgk(@(x) f_sub(x, i, j, k, l, basis, d_basis), 0, 1, tol);
      end
    end
  end
end
...
function q = f_sub(x, i, j, k, l, basis, d_basis)
  tol = 1.e-9;
  q = ones(size(x));
  for p = 1:length(q)
